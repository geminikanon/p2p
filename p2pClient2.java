		...
		private static class CtoSrequest extends Thread
		{
			...
			public void run()
			{
				try{
					...
					String chunknameid = response.substring(response.length()-7, response.length()-4);  // largest number of chunks are 100; do the change;
				    ...
				    chunk.add(chunkname); 
					System.out.println("Client2 has received chunk number " + chunkname);
					...
					File outputfiledir = new File("e:/2015fall/computer network/project/client2/data");
                    if(!outputfiledir.exists())
					{
					      outputfiledir.mkdirs();
			        }
                    // System.out.println("run3 ");
                    File outputfile = new File("e://2015fall/computer network/project/client2/data/" + chunkname +".pdf");
...
//									    {
//									         ask.add(String.valueOf(message.charAt(i)));
//									         
//									    }
//									}
...
//								}
								...
								sendMessage(message);	
								System.out.println("Client1 has sent chunks  " + message + "to Client "+ clientNum);
							}
						   }
...
//		}
		...
		public static void main(String args[]) throws Exception 
		{
			...
			int c = 2;   // define this is client1;
			p2pClient2 client1 = new p2pClient2();
			CtoSrequest ctos1 = new CtoSrequest("localhost", 8082);
			int n = (client1.checkfilenum()) / 5;     // in eg, n=1
			int m = (client1.checkfilenum()) % 5;     // in eg, m=2
			int sizeall = client1.checkfilenum();
			...
			ServerSocket server = new ServerSocket(8020);  // listen to port 8012
			while (true) {  
		         //accept  
		         Socket socket = server.accept();  
		         //every time recevie a new socket;  
		         System.out.println("c1 run2");
		         new Thread(new Task(socket,8020,2)).start();  
		         System.out.println("c1 run3");
		      }  
